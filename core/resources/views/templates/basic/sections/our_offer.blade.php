@php
    ...
    $projects = \App\Models\Project::active()->featured()->available()->beforeEndDate()->take(4)->get();
@endphp
                            ...
                            <article class="card card--offer ">
                                <div class="card-header">
                                    <a class="card-thumb" href="{{ route('project.details', $project->slug) }}">
                                        <img
                                            src="{{ getImage(getFilePath('project') . '/' . $project->image, getFilePath('project')) }}"
                                            alt="{{ __($project->title) }}">
                                    </a>

                                    <div class="card-offer">
                                        <span class="card-offer__label">@lang('ROI')</span>
                                        <span class="card-offer__percentage">{{ __(getAmount($project->roi_percentage)) }}%</span>
                                    </div>
                                </div>
